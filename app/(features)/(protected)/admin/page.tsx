...
type AdminTab = 'members' | 'posts' | 'reports' | 'reviews' | 'manager-approval' | 'system-notification' | 'popup';
...
export default function AdminPage() {
  ...
  const handlePopupImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const safeName = file.name.normalize('NFKD').replace(/[^\w.-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || `file-${Date.now()}`;
  ...
  const resetPopupForm = () => {
    ...
    setPopupForm({ title: '', content: '', image_url: '', link_url: '', link_text: '', is_active: false });
  };
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleTogglePopupActive(popup.popup_id, !popup.is_active)
                        }
                      >
                        {popup.is_active ? (
                          <><XCircle className="size-3.5" /><span className="text-xs">비활성화</span></>
                        ) : (
                          <><CheckCircle2 className="size-3.5 text-emerald-600" /><span className="text-xs">활성화</span></>
                        )}
                      </Button>
                      <Button
                        variant="outline"
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                    ...
                    >
                      {imageUploading ? (
                        ...
                        <>
                          ...
                          <span className="text-[10px] text-muted-foreground/70">JPG, PNG, WEBP 권장</span>
                        </>
                      )}
                    </button>
                  )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-sm font-medium">링크 URL <span className="text-xs text-muted-foreground">(선택)</span></label>
                    <Input
                      placeholder="/post 또는 https://..."
                      value={popupForm.link_url}
                      onChange={(e) => setPopupForm((p) => ({ ...p, link_url: e.target.value }))}
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium">버튼 텍스트 <span className="text-xs text-muted-foreground">(선택)</span></label>
                    <Input
                      placeholder="자세히 보기"
                      value={popupForm.link_text}
                      onChange={(e) => setPopupForm((p) => ({ ...p, link_text: e.target.value }))}
                    />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setPopupForm((p) => ({ ...p, is_active: e.target.checked }))}
                  />
                  <label htmlFor="popup-active" className="text-sm font-medium cursor-pointer">
                    즉시 활성화 <span className="text-xs font-normal text-muted-foreground">(다른 활성 팝업은 자동 비활성화)</span>
                  </label>
                </div>
                <Button
                  onClick={handleSavePopup}
                  disabled={popupSaving || imageUploading || !popupForm.title.trim() || !popupForm.content.trim()}
                  className="w-full"
                >
                  {popupSaving ? (
                ...
                <div className="flex items-center justify-center bg-black/30 rounded-xl p-4 min-h-[300px]">
                  {popupForm.title || popupForm.content || popupForm.image_url ? (
                    ...
                    <div className="flex flex-col items-center gap-2 text-white/50">
                      ...
                      <p className="text-xs">제목이나 내용을 입력하면<br/>미리보기가 표시됩니다</p>
                    </div>
                  )}
                </div>
              </CardContent>
