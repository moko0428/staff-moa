...
- ⚠️ 기존 auth.user 메타데이터와 public.profiles 테이블 간 동기/백필은 추가 점검 필요.
...
- ✅ 매니저 승인 관리 탭: 관리자 페이지에 "매니저 승인 관리" 탭 추가, `profiles`에서 `role='pending_manager'` + `company_verify_status in (null, 'pending')`인 유저만 목록 노출.
...
- ✅ 회원 관리 탭: `profiles`에서 `role != 'admin'`인 유저 목록 조회, 아바타/이름/이메일/role 뱃지 표시, `member`인 경우 근태점수 배지 추가, 이름/이메일 검색 필터.

## 변경/리팩터링 사항

- ♻️ 회원가입 시 매니저 선택 → `role='pending_manager'`, `company_verify_status='pending'`로 저장.
- ♻️ `UserRole` 타입에 `pending_manager` 추가, 관련 타입/스토어 업데이트.
- ♻️ 프로필 페이지에서 `profiles` 테이블 우선 조회하여 role/company_verify_status 등 신뢰성 확보.
...
- ⚠️ `profiles.role` 체크 제약에 `pending_manager` 포함 필요(이미 적용됨).
